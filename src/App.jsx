...
import { Users, Plus, Search, Trash2, Edit2, Eye, EyeOff, Copy, AlertTriangle, CheckCircle, Clock, LogOut, Lock, FileText } from 'lucide-react';
...
const StatCard = ({ title, value, icon: Icon, bgClass, textClass }) => (
    <div className="col-md-6 mb-3"> {/* Cambiado a col-md-6 para ocupar más espacio ya que quitamos ingresos */}
...
export default function App() {
    ...
    const handleSave = async (e) => {
        ...
        const data = { ...formData, renewalDate: renewal.toISOString(), price: parseFloat(formData.price || 0), duration: parseInt(formData.duration), updatedAt: serverTimestamp() };
    ...
    const resetForm = () => {
        setFormData({ clientName: '', service: '', email: '', password: '', price: '', startDate: new Date().toISOString().split('T')[0], duration: '1', notes: '' });
        setEditingId(null);
    };
    ...
    const stats = useMemo(() => {
        let act = 0, exp = 0;
        subs.forEach(s => {
            // Ya no calculamos ingresos aquí
                ...
                <div className="card border-0 shadow-sm">
                    <div className="table-responsive">
                        <table className="table table-hover align-middle mb-0">
                            <thead className="table-light"><tr><th className="ps-3">Cliente</th><th>Credenciales / Notas</th><th>Estado</th><th>Precio</th><th className="text-end pe-3">Acciones</th></tr></thead>
                            <tbody>
                                {filtered.map(s => (
                                    <tr key={s.id}>
                                        ...
                                        <td>
                                            <div className="d-flex flex-column small">
                                                ...
                                                {(s.email || s.password) && (
                                                    <div className="mb-1">
                                                        {s.email && <span className="text-muted d-block">{s.email}</span>}
                                                        {s.password && (
                                                ...
                                                {/* Notas */}
                                                {s.notes && (
                                                    <div className="text-muted fst-italic border-top pt-1 mt-1" style={{ fontSize: '0.85em' }}>
                                                        <FileText size={10} className="me-1" />{s.notes}
                                                    </div>
                                                )}
                                            </div>
                                        </td>
                            ...
                            <div className="modal-body">
                                <form onSubmit={handleSave}>
                                    ...
                                    <div className="mb-2">
                                        ...
                                        <input className="form-control" value={formData.password} onChange={e => setFormData({ ...formData, password: e.target.value })} />
                                    </div>

                                    {/* Notas */}
                                    <div className="mb-3">
                                        <label>Notas / Comentarios</label>
                                        <textarea className="form-control" rows="2" placeholder="Información extra..." value={formData.notes} onChange={e => setFormData({ ...formData, notes: e.target.value })}></textarea>
                                    </div>
